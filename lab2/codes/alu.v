module ALU(
	...
	bit8adder adder(.a(operand1), .b(operand2), .sum(add_result), .cin(1'b0), .cout(add_carry));
	bit8subtractor subtractor(.a(operand1), .b(operand2), .diff(sub_result), .cin(1'b1), .cout(sub_carry));
	bit8adder INC(.a(operand1), .b(8'b1), .sum(inc_result), .cin(1'b0), .cout(inc_carry));
	bit8subtractor DEC(.a(operand1), .b(8'b1), .diff(dec_result), .cin(1'b1), .cout(dec_carry));
	leftShift lshift(.in(operand1), .shift(operand2[2:0]), .out(left_shift_result));
	rightShift rshift(.in(operand1), .shift(operand2[2:0]), .out(right_shift_result));
